...
mod tests {
    ...
    fn events_poll_started_event(participants: Vec<TMAddress>, expires_at: u64) -> EventVerifierEvent {
    ...
    async fn should_vote_not_found_on_malformed_event_data() {
        ...
        assert!(handler.handle(event.try_into().unwrap(), &mut client).await.is_ok());
