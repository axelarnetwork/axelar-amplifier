...
4. If a newer `VerifierSet` was found, a `TransferOperatorship` command is added to the batch. The new `VerifierSet` is
   stored as the next `VerifierSet`.
...
Verifier->>+External Gateway: lookup OperatorshipTransferred event, verify event matches verifier set in poll
...
3. If a newer `VerifierSet` was found, the new `VerifierSet` is stored as the next `VerifierSet`. A `TransferOperatorship`
...
11. The Verifiers see the `PollStarted` event and lookup `OperatorshipTransferred`` event on the External Gateway and
