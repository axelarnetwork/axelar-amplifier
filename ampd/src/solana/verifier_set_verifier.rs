...
use solana_axelar_std::verifier_set::verifier_set_hash;
use solana_sdk::pubkey::Pubkey;
use tracing::error;

use crate::handlers::solana_verify_verifier_set::VerifierSetConfirmation;
use crate::solana::{verify, GatewayEvent};
...
pub fn verify_verifier_set(
    ...
    verify(tx, &message.message_id, gateway_address, |gateway_event| {
        ...
        let Ok(desired_hash) = verifier_set_hash::<Hasher>(&verifier_set, domain_separator) else {
...
/// Transform from Axelar VerifierSet to solana_axelar_encoding VerifierSet
fn to_verifier_set(vs: &VerifierSet) -> Option<solana_axelar_std::verifier_set::VerifierSet> {
...
fn to_pub_key(pk: &PublicKey) -> Option<solana_axelar_std::pubkey::PublicKey> {
    use solana_axelar_std::pubkey::SECP256K1_COMPRESSED_PUBKEY_LEN;
    match pk {
        PublicKey::Ecdsa(hb) => Some(solana_axelar_std::pubkey::PublicKey(
            hb.to_array::<SECP256K1_COMPRESSED_PUBKEY_LEN>().ok()?,
        )),
        // Ed25519 is not supported by solana-axelar-std
        PublicKey::Ed25519(_) => {
            error!("Ed25519 public keys are not supported");
            None
        }
    }
}
...
mod tests {
    ...
    fn fixture_rotate_verifier_set() -> (
        ...
        let expected_hash = solana_axelar_std::verifier_set::verifier_set_hash::<Hasher>(
            &newly_created_vs,
            &DOMAIN_SEPARATOR,
        )
        .unwrap();
