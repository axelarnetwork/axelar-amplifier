...
use prometheus::{Encoder, IntCounter, IntCounterVec, GaugeVec, Registry, TextEncoder, Opts};
use thiserror::Error;
use tracing::warn;
...
impl Metrics {
    pub fn new(registry: &Registry) -> Result<Self, MetricsError> {
        ...
        let (verifier_votes_casted_successful, verifier_votes_casted_failed, verifier_votes_casted_success_rate) = 
    ...
    fn create_verifier_votes_casted_metrics() -> Result<(IntCounterVec, IntCounterVec, GaugeVec), MetricsError> {
        let verifier_votes_casted_successful = IntCounterVec::new(
            Opts::new("verifier_votes_casted_successful", "number of successful votes casted")
        ...
        let verifier_votes_casted_failed = IntCounterVec::new(
            Opts::new("verifier_votes_casted_failed", "number of failed votes casted")
                .variable_labels(vec!["verifier_id".to_string(), "chain_name".to_string()]),
            &["verifier_id", "chain_name"],
        ).change_context(MetricsError::MetricSpawnFailed)?;

        let verifier_votes_casted_success_rate = GaugeVec::new(
            Opts::new("verifier_votes_casted_success_rate", "success rate of votes casted")
                .variable_labels(vec!["verifier_id".to_string(), "chain_name".to_string()]),
            &["verifier_id", "chain_name"],
        ).change_context(MetricsError::MetricSpawnFailed)?;

        Ok((verifier_votes_casted_successful, verifier_votes_casted_failed, verifier_votes_casted_success_rate))
    }
    ...
    pub fn handle_message(&self, msg: MetricsMsg) {
        match msg {
            ...
            MetricsMsg::IncSuccessVoteCasted { verifier_id, chain_name } => {
            ...
            MetricsMsg::IncFailedVoteCasted { verifier_id, chain_name } => {
    ...
    fn update_success_rate(&self, verifier_id: &str, chain_name: &str) {
        let successful_casted_votes = self.verifier_votes_casted_successful
            .with_label_values(&[verifier_id, chain_name])
            .get();
        
        let failed_casted_votes = self.verifier_votes_casted_failed
            .with_label_values(&[verifier_id, chain_name])
            .get();
...
mod tests {
    ...
    fn metrics_handle_all_votes_casted_related_messages_successfully() {
        ...
        assert!(final_metrics.contains("verifier_votes_casted_failed{chain_name=\"ethereum\",verifier_id=\"axelar1abc\"} 1"));
