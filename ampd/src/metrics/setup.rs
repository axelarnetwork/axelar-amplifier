...
mod tests {
    ...
    async fn test_create_metrics_returns_valid_components() {
        ...
        assert!(
            metrics.contains("blocks_received 0"),
            "Initial metrics should show 0 blocks"
        );
    }
    ...
    async fn test_client_server_communication() {
        ...
        for _ in 0..5 {
            client
                .inc_block_received()
                .expect("Failed to increment blocks received");
        }
        tokio::time::sleep(Duration::from_millis(50)).await;
        let metrics = server.gather().expect("Failed to gather metrics");
        assert!(
            metrics.contains("blocks_received 5"),
    ...
    async fn test_client_clone() {
        ...
        client
            .inc_block_received()
            .expect("Failed to increment with original client");
        client_clone
            .inc_block_received()
            .expect("Failed to increment with cloned client");
        tokio::time::sleep(Duration::from_millis(50)).await;
        let metrics = server.gather().expect("Failed to gather metrics");
        assert!(
            metrics.contains("blocks_received 2"),
