...
fn record_transaction_metrics(
    ...
    let duration = broadcast_start.elapsed();

    let status = match success {
        true => "succeeded".to_string(),
        false => "failed".to_string(),
    };

    if let Err(err) = monitoring_client
        .metrics()
        .record_metric(Msg::TransactionProcessed { success, duration })
    {
        warn!(
             err = %err,
             "failed to record metrics for {} transaction broadcast",
             status
        );
    }
}
...
mod tests {
    ...
    async fn should_record_transaction_metrics_for_success_and_failure() {
        let pub_key = random_cosmos_public_key();
        let address = pub_key.account_id(PREFIX).unwrap().into();
        let chain_id: tendermint::chain::Id = "test-chain-id".parse().unwrap();
        let initial_account = create_base_account(&address);
