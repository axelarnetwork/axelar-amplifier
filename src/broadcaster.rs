...
impl<T: BroadcastClient> Broadcaster<T> {
        ...
        M: IntoIterator<Item = cosmrs::Any>,
    {
        ...
        let auth_info_bytes = SignerInfo::single_direct(Some(self.priv_key.public_key()), self.acc_sequence)
            .auth_info(zero_fee())
            .into_bytes()
            .into_report()
            .change_context(BroadcasterError::TxMarshaling)?;
...
fn evaluate_response(response: Result<GetTxResponse, Status>) -> ConfirmationResult {
    match response {
        ...
        Ok(GetTxResponse { tx_response: None, .. }) => {
...
mod tests {
    ...
    async fn gas_estimation_call_failed() {
        ...
        assert!(matches!(
            broadcaster.broadcast(msgs).await.unwrap_err().current_context(),
    ...
    async fn tx_confirmation_failed() {
        ...
        client.expect_broadcast_tx().returning(|_| Ok(TxResponse::default()));
