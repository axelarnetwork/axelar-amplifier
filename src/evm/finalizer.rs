...
where
    C: EthereumClient,
{
    pub fn new<H>(rpc_client: Arc<C>, confirmation_height: H) -> Self
    where
        H: Into<U64>,
    {
        PoWFinalizer {
...
mod tests {
    ...
    async fn latest_finalized_block_height_should_work() {
        ...
        assert_eq!(
            block_number,
            PoWFinalizer::new(Arc::new(rpc_client), 1)
        ...
        assert_eq!(
            block_number + 1,
            PoWFinalizer::new(Arc::new(rpc_client), 0)
                .latest_finalized_block_height()
                .await
                .unwrap()
        );
        ...
        assert_eq!(
            block_number - 1,
            PoWFinalizer::new(Arc::new(rpc_client), 2)
                .latest_finalized_block_height()
                .await
                .unwrap()
        );
        ...
        assert_eq!(
            U64::from(1),
            PoWFinalizer::new(Arc::new(rpc_client), block_number)
                .latest_finalized_block_height()
                .await
                .unwrap()
        );
