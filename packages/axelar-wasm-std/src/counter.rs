...
impl<'a, T: AddAssign + Copy + Default + One + Serialize + DeserializeOwned> Counter<'a, T> {
    ...
    pub fn cur(&self, store: &dyn Storage) -> StdResult<T> {
        Ok(self.item.load(store).unwrap_or_default())
    }
