...
impl TryFrom<Vec<Felt>> for ArraySpan<u8> {
    ...
    fn try_from(data: Vec<Felt>) -> Result<Self, Self::Error> {
        ...
        let arr_length =
            u8::try_from(data[0]).map_err(|e| ArraySpanError::ParsingFelt(e.to_string()))?;
        ...
        let bytes: Result<Vec<u8>, ArraySpanError> = data
            ...
            .iter()
            .copied()
            .map(|data| u8::try_from(data).map_err(|e| ArraySpanError::ParsingFelt(e.to_string())))
            .collect();
...
mod array_span_tests {
    ...
    fn try_from_valid_zeros() {
        // the string "hello", but Felt is bigger than u8::max
        let data = vec![Felt::from_str(
