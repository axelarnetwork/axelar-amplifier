...
pub fn query(
    deps: Deps,
    env: Env,
    msg: TranslationQueryMsg,
) -> Result<Binary, ContractError> {
    match msg {
        TranslationQueryMsg::FromBytes { payload } => {
            query::from_bytes(deps, env, payload)
        }
        TranslationQueryMsg::ToBytes { message } => {
            query::to_bytes(deps, env, message)
        }
    }
}
...
mod tests {
    ...
    use interchain_token_service::{HubMessage, Message, InterchainTransfer, DeployInterchainToken, TokenId};
