...
use interchain_token_service::{HubMessage, Message, TokenId, InterchainTransfer, DeployInterchainToken, LinkToken, RegisterTokenMetadata};
...
pub fn hub_message_abi_decode(payload: &[u8]) -> Result<HubMessage, Error> {
    ...
    let hub_message = match message_type {
        MessageType::SendToHub => {
            let decoded = SendToHubAbi::abi_decode_params(payload)
                .map_err(Error::AbiDecodeFailed)?;
...
mod tests {
    ...
    fn test_message_abi_encode_decode_link_token() {
        let message = Message::LinkToken(LinkToken {
            ...
            source_token_address: nonempty::HexBinary::try_from(vec![0x11, 0x22, 0x33, 0x44]).unwrap(),
            destination_token_address: nonempty::HexBinary::try_from(vec![0x55, 0x66, 0x77, 0x88]).unwrap(),
    ...
    fn test_hub_message_abi_encode_decode_register_token_metadata() {
        let hub_message = HubMessage::RegisterTokenMetadata(RegisterTokenMetadata {
            token_address: nonempty::HexBinary::try_from(vec![0x11, 0x22, 0x33, 0x44, 0x55]).unwrap(),
            decimals: 8,
        });
