...
const MESSAGES_FROM_CONTRACT_CALL: Map<&CrossChainId, Message> = Map::new("sent_messages");
pub(crate) const MESSAGES_FROM_CONTRACT_CALL_COUNTER: Counter<u32> =
    Counter::new("sent_message_counter");
const MESSAGES_TO_EXECUTE: Map<&CrossChainId, ExecutableMessage> = Map::new("received_messages");
...
impl ExecutableMessage {
    pub fn msg(&self) -> &Message {
        match self {
            ExecutableMessage::NotExecuted(msg) | ExecutableMessage::Executed(msg) => msg,
        }
    }
...
pub(crate) fn save_unique_contract_call_msg(
    ...
    if MESSAGES_FROM_CONTRACT_CALL.has(storage, cc_id) {
        return Err(Error::MessageAlreadyExists(cc_id.clone()));
    }

    Ok(MESSAGES_FROM_CONTRACT_CALL.save(storage, cc_id, msg)?)
}

pub(crate) fn may_load_contract_call_msg(
    storage: &dyn Storage,
    id: &CrossChainId,
) -> Result<Option<Message>, Error> {
    Ok(MESSAGES_FROM_CONTRACT_CALL.may_load(storage, id)?)
}

pub fn load_contract_call_msg(storage: &dyn Storage, id: &CrossChainId) -> Result<Message, Error> {
    may_load_contract_call_msg(storage, id)?.ok_or_else(|| Error::MessageNotFound(id.clone()))
}
...
pub(crate) fn save_executable_msg(
    ...
    match existing {
        ...
        None => {
            Ok(MESSAGES_TO_EXECUTE.save(storage, cc_id, &ExecutableMessage::NotExecuted(msg))?)
        }
    }
}
...
pub(crate) fn may_load_executable_msg(
    ...
    Ok(MESSAGES_TO_EXECUTE.may_load(storage, cc_id)?)
}
...
pub(crate) fn mark_msg_as_executed(
    ...
    match existing {
        ExecutableMessage::NotExecuted(msg) => Ok(MESSAGES_TO_EXECUTE.save(
            storage,
            cc_id,
