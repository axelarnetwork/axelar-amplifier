...
pub fn get_active_worker_set(deps: Deps, prover_address: Addr) -> Result<WorkerSet, ContractError> {
    ACTIVE_WORKERSET_FOR_PROVER
        .may_load(deps.storage, prover_address.clone())?
        .ok_or(ContractError::NoActiveWorkerSetRegistered)
}
