...
mod tests {
    ...
    pub fn setup_test_case() -> OwnedDeps<MockStorage, MockApi, MockQuerier, Empty> {
        ...
        instantiate(
            ...
            InstantiateMsg {
                ...
                gateway_address: router_api::GATEWAY_COSMOS_ADDR.clone().to_string(),
                ...
                service_registry_address: router_api::SERVICE_REGISTRY_COSMOS_ADDR
                    .clone()
                    .to_string(),
                voting_verifier_address: router_api::VOTING_VERIFIER_COSMOS_ADDR
                    .clone()
                    .to_string(),
    ...
    fn execute_construct_proof(
        ...
        execute(
            deps,
            mock_env(),
            message_info(&router_api::RELAYER_COSMOS_ADDR.clone(), &[]),
            msg,
        )
    }
    ...
    fn test_instantiation() {
        let instantiator = router_api::INSTANTIATOR_COSMOS_ADDR.clone();
        let admin = router_api::ADMIN_COSMOS_ADDR.clone();
        let governance = router_api::GOVERNANCE_COSMOS_ADDR.clone();
        let gateway_address = router_api::GATEWAY_COSMOS_ADDR.clone();
        let multisig_address = router_api::MULTISIG_COSMOS_ADDR.clone();
        let coordinator_address = router_api::COORDINATOR_COSMOS_ADDR.clone();
        let service_registry_address = router_api::SERVICE_REGISTRY_COSMOS_ADDR.clone();
        let voting_verifier_address = router_api::VOTING_VERIFIER_COSMOS_ADDR.clone();
    ...
    fn test_confirm_verifier_set_unconfirmed() {
        ...
        let res = confirm_verifier_set(deps.as_mut(), router_api::RELAYER_COSMOS_ADDR.clone());
        assert!(res.is_err());
        assert_eq!(
            res.unwrap_err().to_string(),
