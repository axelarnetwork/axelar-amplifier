...
use multiversx_sc_codec::{
    dep_encode_to_vec, DecodeError, DecodeErrorHandler, EncodeErrorHandler, NestedDecodeInput,
    NestedEncode, NestedEncodeOutput,
};
...
impl WeightedSigners {
    ...
    pub fn top_encode(self) -> Result<Vec<u8>, ContractError> {
        Ok(
    ...
    pub fn dep_encode(self) -> Result<Vec<u8>, ContractError> {
        Ok(
            dep_encode_to_vec(&(self.signers, self.threshold.as_slice(), self.nonce))
                .expect("couldn't serialize weighted signers as mvx"),
        )
    }
}
...
fn encode(payload: &Payload) -> Result<Vec<u8>, ContractError> {
    match payload {
        ...
        Payload::VerifierSet(verifier_set) => {
            ...
            let mut weighted_signers = WeightedSigners::from(verifier_set).top_encode()?;
...
mod tests {
    ...
    use crate::encoding::mvx::WeightedSigner;
    use crate::test::test_data::{
        curr_verifier_set_ed25519, domain_separator_other, messages_mvx,
