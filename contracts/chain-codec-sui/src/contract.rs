...
const CONTRACT_NAME: &str = "crates.io:chain-codec-sui";
...
pub fn query(_deps: Deps, _env: Env, msg: QueryMsg) -> Result<Binary, ContractError> {
    Ok(match msg {
        QueryMsg::PayloadDigest { domain_separator, signer, payload } => {
            to_json_binary(&crate::bcs::payload_digest(&domain_separator, &signer, &payload)?)?
        }
        QueryMsg::EncodeExecData { domain_separator, verifier_set, signers, payload } => {
            to_json_binary(&crate::bcs::encode_execute_data(&domain_separator, &verifier_set, signers, &payload)?)?
        }
