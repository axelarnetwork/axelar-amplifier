...
pub const KEY_SETS: Map<u64, KeySet> = Map::new("key_sets");
pub const CURRENT_KEY_SET_ID: Map<&Addr, Uint64> = Map::new("current_key_set_id");

pub fn get_current_key_set(store: &mut dyn Storage, owner: Addr) -> Result<KeySet, ContractError> {
    ...
    KEY_SETS
        .load(store, key_set_id.u64())
        .map_err(|_| ContractError::NoActiveKeyFound { owner })
}
