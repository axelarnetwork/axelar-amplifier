...
use crate::types::{Data, KeccackHash, Message};
...
impl CommandBatch {
    pub fn new(block_height: u64, messages: Vec<Message>, destination_chain_id: Uint256) -> Self {
        ...
        let data = build_commands_data(destination_chain_id, messages);

        let encoded_data = data.encode();

        let id = build_batch_id(block_height, &encoded_data);
        let msg_to_sign = build_msg_to_sign(&encoded_data);
...
fn build_commands_data(destination_chain_id: Uint256, messages: Vec<Message>) -> Data {
...
fn build_msg_to_sign(data: &HexBinary) -> HexBinary {
...
mod tests {
    ...
    fn test_new_batch() {
        ...
        let expected_batch = CommandBatch {
            id: hex::decode("21871578588b38282f449232db51a47b9e22289ddf3f7052e03c7b464ab1b84b")
                .unwrap()
                .try_into()
                .unwrap(),
            message_ids: vec![
                "c8a0024fa264d538986271bdf8d2901c443321faa33440b9f28e38ea28e6141f-1".into(),
                "e7a7263a63ac449b4c6ce2a93accfae9ae49c1d96e3fa9c19cc417130bcfda22-1".into(),
            ],
            data: Data {
                destination_chain_id,
                commands_ids: vec![
                    hex::decode("3ecc0a06d74c403d85a4953742287ab3bed54be997314a2bbd30f3c5cd3666fa")
                        .unwrap()
                        .try_into()
                        .unwrap(),
                    hex::decode("96c1b646d876886426e9e9dd5e4acc96cd87918b0d2685f7e6e5881ca35a6630")
                        .unwrap()
                        .try_into()
                        .unwrap(),
                ],
                commands_types: vec!["approveContractCall".to_string(), "approveContractCall".to_string()],
                commands_params: vec![
                    HexBinary::from_hex("").unwrap(),
                    HexBinary::from_hex("").unwrap(),
                ],
            },
            msg_to_sign: hex::decode(
                "3531ca948047f8f391f1f3e768bd5e47e372d19663c4408e0d5a1fd88d1af8f0",
            )
            .unwrap()
            .try_into()
            .unwrap(),
        ...
        assert_eq!(batch.message_ids, expected_batch.message_ids);
        assert_eq!(batch.data, expected_batch.data);
        assert_eq!(batch.msg_to_sign, expected_batch.msg_to_sign);
    }
    ...
    fn test_msg_to_sign() {
        ...
        let actual = hex::encode(build_msg_to_sign(&data));
