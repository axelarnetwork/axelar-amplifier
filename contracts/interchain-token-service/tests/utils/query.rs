...
use interchain_token_service::{TokenConfigMsg, TokenId, TokenInstanceMsg};
...
pub fn query_token_instance(
    ...
    chain: ChainNameRaw,
    token_id: TokenId,
) -> Result<Option<TokenInstanceMsg>, ContractError> {
    let bin = query(
        deps,
        mock_env(),
...
pub fn query_token_config(
    deps: Deps,
    token_id: TokenId,
) -> Result<Option<TokenConfigMsg>, ContractError> {
