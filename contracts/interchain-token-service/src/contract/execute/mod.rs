...
pub fn execute_message(
    ...
    let hub_message = translate_from_bytes(deps.storage, deps.querier, &cc_id.source_chain, &payload)?;
...
fn execute_message_on_hub(
    ...
    let destination_payload = translate_to_bytes(deps.storage, deps.querier, &destination_chain, &hub_message)?;
...
fn translate_from_bytes(
    ...
    let chain_config = state::load_chain_config(storage, source_chain)
        .change_context(Error::State)?;
    
    // Query the translation contract to decode the payload
    let query_msg = crate::msg::TranslationQueryMsg::FromBytes {
        payload: payload.clone(),
    };
    
    let result = querier
        .query_wasm_smart(&chain_config.translation_contract.to_string(), &query_msg)
        .change_context(Error::TranslationFailed);
    match result {
        Ok(response) => Ok(response),
        Err(e) => {
            // Extract the inner StdError from the error_stack::Report
            let msg = format!("{:#}", e); // pretty print the error chain
            println!("DEBUG: Translation error message: {}", msg);
            if msg.contains("Querier system error: Cannot parse request: Translation failed") {
                Err(Error::InvalidPayload.into())
            } else {
                Err(e)
            }
        }
...
fn translate_to_bytes(
    ...
    let chain_config = state::load_chain_config(storage, destination_chain)
        .change_context(Error::State)?;
    
    // Query the translation contract to encode the message
    let query_msg = crate::msg::TranslationQueryMsg::ToBytes {
        message: hub_message.clone(),
    };
    
    let response: HexBinary = querier
        .query_wasm_smart(&chain_config.translation_contract.to_string(), &query_msg)
        .change_context(Error::TranslationFailed)?;
