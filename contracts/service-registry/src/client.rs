...
pub enum Error {
    #[error("failed to query the service registry for active verifiers for service {service_name} and chain {chain_name}")]
    QueryActiveVerifiers {
        service_name: String,
        chain_name: ChainName,
    },

    #[error("failed to query the service registry for service {0}")]
    QueryService(String),

    #[error(
        "failed to query the service registry for verifier {verifier} of service {service_name}"
    )]
    QueryVerifier {
        service_name: String,
        verifier: String,
