...
use axelar_wasm_std::nonempty::Usize;
...
pub fn migrate(
    ...
    let services = services(
        deps.storage,
        None,
        Usize::try_from(usize::MAX).change_context(MigrationError::InvalidLimit)?,
    )
    .change_context(ContractError::ServiceNotFound)?;

    for s in services {
        let _ = update_authorized_verifier_count(deps.storage, &s.name)?;
    }
