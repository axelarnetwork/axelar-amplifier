...
use integration_tests::protocol::{CustomApp, CustomNexusQuerier, Protocol};
...
pub fn route_messages(app: &mut CustomApp, gateway: &GatewayContract, msgs: &[Message]) {
    let response = gateway.execute(
        app,
        Addr::unchecked("relayer"),
...
pub fn end_poll(app: &mut CustomApp, voting_verifier: &VotingVerifierContract, poll_id: PollId) {
    let response = voting_verifier.execute(
        app,
        Addr::unchecked("relayer"),
...
pub fn advance_height(app: &mut CustomApp, increment: u64) {
...
pub fn advance_at_least_to_height(app: &mut CustomApp, desired_height: u64) {
    let cur_block = app.block_info();
...
pub fn setup_protocol(service_name: nonempty::String) -> Protocol {
    ...
    let mut app = AppBuilder::new_custom()
        .with_custom(CustomNexusQuerier {
            tx_hash_and_nonce: Box::new(|_| unimplemented!()),
            is_chain_registered: Box::new(|_| unimplemented!()),
        })
...
pub fn query_balance(app: &CustomApp, address: &Addr) -> Uint128 {
    app.wrap()
...
pub fn query_balances(app: &CustomApp, verifiers: &Vec<Verifier>) -> Vec<Uint128> {
